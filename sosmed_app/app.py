...
MIN_FOLLOWERS_FOR_VERIFIED = 1  # Syarat minimal followers untuk verified
...
def search():
    ...
    # Get search history untuk user saat ini
    history_records = conn.execute(
        "SELECT DISTINCT query FROM search_history WHERE user_id = ? ORDER BY created_at DESC LIMIT 10",
        (session['user_id'],)
    ).fetchall()
    
    search_history = [record['query'] for record in history_records]
    
    if query:
        # Simpan ke search history jika tidak kosong
        try:
            conn.execute(
            ...
            pass  # Ignore duplicate atau error lainnya
        ...
        for user in users:
            ...
            user_dict = dict(user)
            user_dict['is_verified'] = followers_count >= MIN_FOLLOWERS_FOR_VERIFIED
...
# Tambahkan route baru untuk menghapus riwayat pencarian
@app.route("/clear-search-history", methods=["POST"])
def clear_search_history():
    if "username" not in session:
        return jsonify({"success": False, "message": "Harus login"}), 401
    ...
    return jsonify({"success": True, "message": "Riwayat pencarian berhasil dihapus"})

# Tambahkan route untuk menghapus item riwayat tertentu
@app.route("/remove-search-item", methods=["POST"])
def remove_search_item():
    ...
    if not query:
        return jsonify({"success": False, "message": "Query tidak valid"}), 400
    
    conn = get_db_connection()
    conn.execute("DELETE FROM search_history WHERE user_id = ? AND query = ?", 
                (session['user_id'], query))
    conn.commit()
    conn.close()
...
def profile(username):
    ...
    # Get follow stats untuk menentukan verified status
    followers_count = conn.execute(
        "SELECT COUNT(*) FROM follows WHERE following_id = ?",
        (user['id'],)
    ).fetchone()[0]
    
    # Determine if profile owner is verified
    is_profile_verified = followers_count >= MIN_FOLLOWERS_FOR_VERIFIED
    
    # Get user posts dengan informasi verified untuk setiap post
    posts = conn.execute(
        "SELECT * FROM posts WHERE user_id = ? ORDER BY created_at DESC",
        (user['id'],)
    ).fetchall()
    
    # Karena semua post di profile adalah milik user yang sama,
    # kita bisa menggunakan is_profile_verified untuk semua post
    posts_with_likes = []
    for post in posts:
        # Get like count
        ...
        post_dict['is_verified'] = is_profile_verified  # Semua post dari user yang sama
    ...
    return render_template("profile.html", 
                          ...
                          is_verified=is_profile_verified,  # Untuk header profile
...
def feed():
    ...
    # Get like information and verification status for each post
    posts_with_likes = []
    for post in posts:
        # Get like count
        like_count = conn.execute(
        ...
        # Check if post author is verified (has at least 1 follower)
        author_followers = conn.execute(
            "SELECT COUNT(*) FROM follows WHERE following_id = ?",
            (post['user_id'],)
        ).fetchone()[0]
        
        is_verified = author_followers >= MIN_FOLLOWERS_FOR_VERIFIED
        
        post_dict = dict(post)
        post_dict['like_count'] = like_count
        post_dict['is_liked'] = is_liked
        post_dict['comment_count'] = comment_count
        post_dict['is_verified'] = is_verified
        
        posts_with_likes.append(post_dict)
    
    conn.close()
    
    return render_template("feed.html", posts=posts_with_likes, format_datetime=format_datetime)
...
def get_comments(post_id):
    ...
    for comment in comments:
        # Check if comment author is verified (has at least 1 follower)
        author_followers = conn.execute(
            "SELECT COUNT(*) FROM follows WHERE following_id = ?",
            (comment['user_id'],)
        ).fetchone()[0]
        ...
        comments_list.append({
            ...
            'created_at': format_datetime(comment['created_at'], 'relative'),
            'is_verified': is_verified  # Tambahkan status verified
        })
...
def add_comment(post_id):
    ...
    comment_data = {
        ...
        'display_name': new_comment['display_name'] or new_comment['username'],
        'profile_picture': new_comment['profile_picture'],
        'content': new_comment['content'],
        'created_at': format_datetime(new_comment['created_at'], 'relative')
    }
